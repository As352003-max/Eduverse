...
router.post('/progress', protect, async (req, res) => {
    ...
    try {
        const gameProgress = await GameProgress.findOneAndUpdate(
            { userId, moduleId },
            { $set: { progress, score, completed, hintsUsed, customData, lastAttemptedAt: Date.now() }, $inc: { attempts: 1 } },
            { upsert: true, new: true, setDefaultsOnInsert: true }
        );

        let gamificationResult = {};
        if (completed || progress >= 50) gamificationResult = await applyGamificationLogic(userId, score, completed);

        res.status(200).json({ message: 'Game progress updated successfully.', gameProgress, gamification: gamificationResult });
    } catch (error) {
...
router.get('/progress/:moduleId', protect, async (req, res) => {
    ...
    try {
        ...
        if (!gameProgress) return res.status(404).json({ message: 'No progress found for this module.' });
